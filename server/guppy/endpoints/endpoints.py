...
def _is_valid_band(src, band: int) -> bool:
    return isinstance(band, int) and 1 <= band <= src.count


def _get_nodata_for_band(src, band: int):
    if src.nodatavals and len(src.nodatavals) >= band and src.nodatavals[band - 1] is not None:
...
def get_stats_for_bbox(db: Session, layer_name: str, bbox_left: float, bbox_bottom: float, bbox_right: float, bbox_top: float, native: bool, band: int = 1):
    t = time.time()
...
def get_min_max_for_layer(layer_name: str, db: Session, band: int = 1):
    t = time.time()
    layer_model = db.query(m.LayerMetadata).filter_by(layer_name=layer_name).first()
    if layer_model:
        ...
        if os.path.exists(path):
            ...
            with Reader(path) as reader:
                ...
                band_stats = stats_by_band.get(f"b{band}") if stats_by_band else None
...
def get_data_for_wkt(db: Session, layer_name: str, body: s.GeometryBody, all_touched: bool = False, band: int = 1):
    t = time.time()
    layer_model = db.query(m.LayerMetadata).filter_by(layer_name=layer_name).first()
    if layer_model:
        path = layer_model.file_path if not layer_model.is_mbtile else layer_model.data_path
            ...
            if geom.is_valid:
                ...
                if geom.is_valid:
                    with rasterio.open(path) as src:
                        ...
                        try:
                            rst, _ = _extract_area_from_dataset(src, [geom], crop=True, all_touched=all_touched, is_rgb=layer_model.is_rgb, band=band)
                            if layer_model.is_rgb:
                                rst = _decode(rst)
...
def get_stats_for_wkt(db: Session, layer_name: str, body: s.GeometryBody, native: bool, band: int = 1):
    t = time.time()
    layer_model = db.query(m.LayerMetadata).filter_by(layer_name=layer_name).first()
    if layer_model:
        path = layer_model.file_path if not layer_model.is_mbtile else layer_model.data_path
                ...
                if geom.is_valid:
                    ...
                    with rasterio.open(path, overview_level=overview_factor) as src:
                        if not _is_valid_band(src, band):
                            return Response(content=f"invalid band {band}. available range: 1-{src.count}", status_code=status.HTTP_406_NOT_ACCEPTABLE)
                        try:
                            rst, _ = _extract_area_from_dataset(src, [geom], crop=True, is_rgb=layer_model.is_rgb, band=band)
                            if layer_model.is_rgb:
                                rst = _decode(rst)
                    ...
                    if rst.size != 0:
                        ...
                        if shape_mask[shape_mask == 0].size < 50:
                            ...
                            response = create_stats_response(rst, shape_mask, nodata,
...
def get_stats_for_model(layer_model, native, geom, srs, band: int = 1):
    """
    ...
    if os.path.exists(path) and geom:
        ...
        if geom.is_valid:
            ...
            if geom.is_valid:
                ...
                if rst.size != 0:
                    nodata = _get_nodata_for_band(src, band)
                    return create_stats_response(rst, shape_mask, nodata,
                                                 type=f'stats wkt. Overview level: {overview_factor}, {overview_bin} scale',
                                                 layer_name=layer_model.layer_name
                                                 )
        logger.info(f'classification_for_wkt 406 invalid geometry {time.time() - t}')
...
def get_stats_for_wkt_list(db: Session, body: s.GeometryBodyList, native: bool, band: int = 1):
    """
    Args:
    ...
    * layer metadata is not found, returns a HTTP 404 response.
    ...
    if layer_models:
        ...
        result = Parallel(n_jobs=-1, prefer='threads')(delayed(get_stats_for_model)(layer_model, native, geom, srs, band) for layer_model in layer_models)
        if result:
...
def get_line_data_for_wkt(db: Session, layer_name: str, body: s.LineGeometryBody, band: int = 1):
    t = time.time()
    layer_model = db.query(m.LayerMetadata).filter_by(layer_name=layer_name).first()
    if layer_model:
        path = layer_model.file_path if not layer_model.is_mbtile else layer_model.data_path
...
def get_line_data_list_for_wkt(db: Session, body: s.LineGeometryListBody, band: int = 1):
    t = time.time()
    layer_models = db.query(m.LayerMetadata).filter(m.LayerMetadata.layer_name.in_(body.layer_names)).all()
    coords = {}
    if layer_models:
        ...
        if coords:
            ...
            result = Parallel(n_jobs=-1, prefer='threads')(delayed(sample_coordinates)(coords['1'], layer_model.file_path, layer_model.layer_name, band) for layer_model in layer_models)
            if result:
...
def get_multi_line_data_list_for_wkt(db: Session, body: s.MultiLineGeometryListBody, band: int = 1):
    t = time.time()
    layer_models = db.query(m.LayerMetadata).filter(m.LayerMetadata.layer_name.in_(body.layer_names)).all()
    coords_list = {}
    result_per_line = []
    ...
    if layer_models:
        ...
        if coords_list:
            ...
            result = sample_coordinates_window(coords_list, layer_models, MultiLineString(epsg_lines).bounds, body.round_val, band=band)
            start_data = 0
...
def get_point_value_from_layer(db: Session, layer_name: str, x: float, y: float, band: int = 1):
    t = time.time()
    layer_model = db.query(m.LayerMetadata).filter_by(layer_name=layer_name).first()
    ...
    if layer_model and is_raster:
        ...
        if os.path.exists(path) and x and y:
            with rasterio.open(path) as src:
                ...
                for v in src.sample([(x_, y_)], indexes=band):
        ...
        if os.path.exists(path) and x and y:
            ...
            values = tile_df[tile_df.intersects(point)]
            values["geometry"] = values.geometry.to_wkt()
            if not values.empty:
...
def get_classification_for_wkt(db: Session, layer_name: str, body: s.GeometryBody, band: int = 1):
    t = time.time()
    layer_model = db.query(m.LayerMetadata).filter_by(layer_name=layer_name).first()
    if layer_model:
        path = layer_model.file_path if not layer_model.is_mbtile else layer_model.data_path
                    ...
                    with rasterio.open(path) as src:
                        ...
                        if geom.area / (src.res[0] * src.res[1]) > cfg.guppy.size_limit:
                            return Response(content=f'geometry area too large ({geom.area}m². allowed <={cfg.guppy.size_limit * (src.res[0] * src.res[1])}m²)',
                                            status_code=status.HTTP_406_NOT_ACCEPTABLE)
                        try:
                            rst, crop_transfrom = _extract_area_from_dataset(src, [geom], crop=True, is_rgb=layer_model.is_rgb, band=band)
                            if layer_model.is_rgb:
                                rst = _decode(rst)
                            shape_mask = _extract_shape_mask_from_dataset(src, [geom], crop=True)
                        except ValueError as e:
                    ...
                    if rst.size != 0:
                        if shape_mask[shape_mask == 0].size < 50:
                            with rasterio.open(path) as src:
                                if not _is_valid_band(src, band):
                                    return Response(content=f"invalid band {band}. available range: 1-{src.count}", status_code=status.HTTP_406_NOT_ACCEPTABLE)
                                rst, crop_transfrom = _extract_area_from_dataset(src, [geom], crop=True, all_touched=True, is_rgb=layer_model.is_rgb, band=band)
                                if layer_model.is_rgb:
                                    rst = _decode(rst)
...
def get_countour_for_models(db: Session, body: s.CountourBodyList, band: int = 1):
    ...
    layer_models = db.query(m.LayerMetadata).filter(m.LayerMetadata.layer_name.in_(body.layer_names)).all()
    if layer_models:
        result = Parallel(n_jobs=-1, prefer='threads')(delayed(get_layer_contour)(layer, band) for layer in layer_models)
        if result:
...
def get_quantiles_for_wkt(db: Session, layer_name: str, body: s.QuantileBody, native: bool, band: int = 1):
    t = time.time()
    layer_model = db.query(m.LayerMetadata).filter_by(layer_name=layer_name).first()
    if layer_model:
        ...
        if os.path.exists(path) and body:
            ...
            if geom.is_valid:
                ...
                if geom.is_valid:
                    ...
                    if rst.size != 0:
                        nodata = _get_nodata_for_band(src, band)
                        response = create_quantile_response(rst, nodata,
